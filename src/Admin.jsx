    ...
    <div className="ve-workspace">
      <aside className="ve-sidebar">
        ...
        {panel === "home" && <section className="ve-panel">
          ...
          {content.news.map((item, index) => <div className="ve-pair" key={index}><div><Field label={`Date ${index + 1}`} value={item.date} onChange={(value) => update({ ...content, news: content.news.map((news, i) => i === index ? { ...news, date: value } : news) })} /><label style={{ display: "flex", alignItems: "center", gap: 7, margin: "-4px 0 12px", color: "var(--ve-muted)", fontSize: 11 }}><input type="checkbox" checked={item.visible !== false && item.dateVisible !== false} onChange={(event) => update({ ...content, news: content.news.map((news, i) => i === index ? { ...news, visible: event.target.checked, dateVisible: event.target.checked } : news) })} /> Show news {index + 1}</label></div><Field label="Text" value={item.text} onChange={(value) => update({ ...content, news: content.news.map((news, i) => i === index ? { ...news, text: value } : news) })} /></div>)}
        </section>}
